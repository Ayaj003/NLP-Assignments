...
glove_embeddings = KeyedVectors.load_word2vec_format(word2vec_output_file, binary=False)
...
word2vec_embedding_matrix = create_embedding_matrix(word2vec_embeddings, tokenizer, embedding_dim)
glove_embedding_matrix = create_embedding_matrix(glove_embeddings, tokenizer, embedding_dim)
...
word2vec_scores = word2vec_model.evaluate(X_val_array, y_val, verbose=0)
word2vec_predictions = np.argmax(word2vec_model.predict(X_val_array), axis=-1)
word2vec_result = classification_report(y_val, word2vec_predictions, output_dict=True)

glove_scores = glove_model.evaluate(X_val_array, y_val, verbose=0)
glove_predictions = np.argmax(glove_model.predict(X_val_array), axis=-1)
